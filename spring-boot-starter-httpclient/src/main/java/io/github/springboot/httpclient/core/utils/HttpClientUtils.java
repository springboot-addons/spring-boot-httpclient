...
public final class HttpClientUtils {
	...
	public static URI getUri(HttpHost httpHost, HttpContext httpContext) throws IOException {
		try {
			return new URI(getSb().append(httpHost.getSchemeName()).append("://").append(httpHost.getHostName())
					.append(":").append(httpHost.getPort()).toString());
	...
	private static Request addContent(Request request, final Map<String, Object> content, final boolean multiPart)
		...
		if (!multiPart && (cs != null)) {
			if (cs instanceof InputStream) {
				he = new ProgressEntityWrapper(
						new InputStreamEntity((InputStream) cs, contentLength, effectiveContentType), pListener);
...
//            final String fieldName = StringUtils.substringBeforeLast(inputContent.getName(), ".");
						...
						multipartEntityBuilder.addBinaryBody(entry.getKey(), (InputStream) entry.getValue(),
								effectiveContentType, entry.getKey());
					} else if (entry.getValue() instanceof byte[]) {
						multipartEntityBuilder.addBinaryBody(entry.getKey(), (byte[]) entry.getValue(),
								effectiveContentType, entry.getKey());
					} else {
						multipartEntityBuilder.addTextBody(entry.getKey(), (String) entry.getValue(),
								textDefaultContentType);
					}
				}
			}
