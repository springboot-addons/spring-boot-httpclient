...
public class HeadersPropagationInterceptor implements Filter, HttpRequestInterceptor, HttpResponseInterceptor {
            ...
            try {
                for (Header h : response.getAllHeaders()) {
                    if (config.getUpPattern().matcher(h.getName()).matches()) {
                        ...
                        upHeadersProvider.ifAvailable(requestHeadersStorage -> requestHeadersStorage.add(h.getName(), h.getValue()));
                    }
                }
            } catch (Exception e) {
                // ignore
    ...
    public void process(HttpRequest request, HttpContext context) {
        ...
        if (config.isEnabledPropagation()) {
            try {
                downHeadersProvider.ifAvailable(requestHeadersStorage ->
                        requestHeadersStorage.getHeaderList().stream()
                                .filter(h -> config.getDownPattern().matcher(h.getName()).matches())
                                .forEach(h -> {
                                    log.debug("*** Using header {} from storage for subsequent call to {}", h, request.getRequestLine());
                                    request.addHeader(h);
                                }));
            } catch (Exception e) {
                // ignore
        ...
        if (request instanceof HttpServletRequest) {
            ...
            while (headerNames.hasMoreElements()) {
                ...
                if (config.getDownPattern().matcher(headerName).matches()) {
                    ...
                    downHeadersProvider.ifAvailable(requestHeadersStorage -> requestHeadersStorage.add(headerName, hRequest.getHeaders(headerName)));
                }
            }
            ...
            upHeadersProvider.ifAvailable(requestHeadersStorage ->
                    requestHeadersStorage.getHeaderList().stream()
                            .filter(eh -> config.getUpPattern().matcher(eh.getName()).matches())
                            .forEach(eh -> {
